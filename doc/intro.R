...
if(res@Number!=0){
  ...
  for(jj in 1:res@Number){
    ...
    bic.out[["row"]][[paste0("clus",jj)]] <- as.numeric(na.omit(as.numeric(unlist(strsplit(rownames(bic), split="Cell")))))
    bic.out[["col"]][[paste0("clus",jj)]] <- as.numeric(na.omit(as.numeric(unlist(strsplit(colnames(bic), split="Gene")))))
  }
}
